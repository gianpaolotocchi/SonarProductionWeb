...
const SliderDesktop = () => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const sliderRef = useRef(null);
  const [cardWidth, setCardWidth] = useState(0); // Stato per la larghezza della card
  ...
  useEffect(() => {
    // Calcola la larghezza della card al montaggio del componente
    const card = sliderRef.current.querySelector(".card");
    if (card) {
      setCardWidth(card.offsetWidth); // Imposta la larghezza della card
    }
  }, []);

  const handleScroll = (direction) => {
    if (!sliderRef.current) return; // Verifica se sliderRef è disponibile

    let newIndex = currentIndex + direction;

    // Calcola il nuovo indice per schermi piccoli
    if (window.innerWidth <= 768) {
      newIndex = currentIndex + direction * 1; // Sposta di 1 card
    }
